...
try:
	...
	from http_check_uri import message
...
class Test_http_check_uri():
	...
	@allure.feature('验证基于uri方法的网页访问策略的过滤')
	def test_http_check_uri_a1(self):
		...
		# 发送get请求，请求内容包含黑名单
		status_code = http_check.http_get(self.case1_uri,self.data)
		print('get请求内容包含黑名单返回的状态码为：{}'.format(status_code))
		assert status_code == 403

		# 发送post请求，请求内容包含黑名单
		status_code = http_check.http_post(self.case1_uri, self.data)
		print('get请求内容包含黑名单返回的状态码为：{}'.format(status_code))
		assert status_code == 403

	# @pytest.mark.skip(reseason="skip")
	@allure.feature('验证基于uri方法的多个网页访问策略的过滤')
