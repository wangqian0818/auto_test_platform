...
pkt1_cfg={
    "hping3":[f"hping3 -I {ciface} -a {pcap_dip} -S {pcap_dip} -p {attack_port} -i u1000"],
...
pkt2_cfg={
    "hping3":[f"hping3 -I ens9 -S -p {attack_port} {pcap_dip} --rand-source -i u1000"],
    "http":[f"python2.7 -m SimpleHTTPServer {dport}"],
    #"send":[ciface,1,"0001_acl.pcap"],
    "capture":[siface,f"tcp and host {pcap_dip}",1,"test_ddos_randomSipSport_syn_flood.pcap"],
    "read":["test_ddos_randomSipSport_syn_flood.pcap",0],
    "expect":[strip,0,0]
}
