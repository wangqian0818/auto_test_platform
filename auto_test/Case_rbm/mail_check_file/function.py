...
try:
	...
	from mail_check_file import message
...
class Test_mail_check_file():
	...
	@allure.feature('验证基于文件名过滤的邮件策略')
	def test_mail_check_file_a1(self):
		...
		print('黑名单文件名{}结果为:{}'.format(self.case1_file1,result1))
		assert result1 == 0
		...
		print('非黑名单文件名{}结果为:{}'.format(self.case1_file2,result2))
		assert result2 == 1
	...
	def test_mail_check_file_a2(self):
		...
		print('第一个黑名单文件名{}结果为:{}'.format(self.case2_file1,result1))
		assert result1 == 0

		result2 = send_smtp.post_email(self.mail_sender, self.mail_receivers, self.mail_cc, self.mail_bcc,
									   self.mail_host, self.mail_port, self.mail_user, self.mail_pass,
									   self.case2_attach2, self.case2_file2, self.case2_title, self.context, 0, 1)
		print('第二个黑名单文件名{}结果为:{}'.format(self.case2_file2,result2))
		assert result2 == 0
		...
		print('非黑名单文件名{}结果为:{}'.format(self.case2_file3,result3))
		assert result3 == 1
