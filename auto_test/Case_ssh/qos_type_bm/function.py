...
class Test_qos_type_bm():
	...
	@allure.feature('验证qos限速类型为广播报文时对UDP的限速情况')
	def test_qos_type_bm_a1(self):
		...
		s_speed = fun.qos_speed('qos_type_bm_a1.txt', s_txt, qbucket='s')
		assert 9.0 <= float(s_speed) <= 10.0


	@allure.feature('验证qos限速类型为组播播报文时对TCP和UDP的限速情况')
	def test_qos_type_bm_a2(self):
		...
		# 检查速率是否正确
		s_txt = fun.cmd('cat s.txt', 's')
		s_speed = fun.qos_speed('qos_type_bm_a2.txt', s_txt, qbucket='s')
		assert 9.0 <= float(s_speed) <= 10.0
