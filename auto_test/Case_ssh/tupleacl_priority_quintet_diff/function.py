...
print(base_path)
try:
	from tupleacl_priority_quintet_diff import index
...
class Test_acl_priority_quintet_diff():
	...
	@allure.feature('验证优先级相同情况下的策略命中规则（五元组数量不同,比较sip+dip和sip+dip+dp）')
	def test_acl_priority_quintet_diff_a1(self):
	...
	# @pytest.mark.skip(reseason="skip")
	@allure.feature('验证优先级相同情况下的策略命中规则（五元组数量不同,比较dip和dip+协议）')
	def test_acl_priority_quintet_diff_a2(self):
