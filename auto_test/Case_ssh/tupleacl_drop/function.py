...
try:
	from tupleacl_drop import index
except Exception as err:
	print(
...
class Test_acl_drop():
	...
	def test_acl_drop_a1(self):
		...
		#初始化
		cap_iface, cap_filter, cap_num, cap_pcap1, cap_pcap2, cap_pcap3 = self.pkt1_cfg["capture"][0], self.pkt1_cfg["capture"][1], \
															   self.pkt1_cfg["capture"][2], self.pkt1_cfg["capture"][3], \
															   self.pkt1_cfg['capture'][4], self.pkt1_cfg['capture'][5]
		c_iface, c_num, c_pcap = self.pkt1_cfg["send"][0], self.pkt1_cfg["send"][1], self.pkt1_cfg["send"][2]
		read_name1, read_name2, read_name3, read_id = self.pkt1_cfg["read"][0], self.pkt1_cfg["read"][1], self.pkt1_cfg['read'][2], self.pkt1_cfg['read'][3]
	...
	def test_acl_drop_a2(self):

		# 初始化
		cap_iface, cap_filter, cap_num, cap_pcap1, cap_pcap2, cap_pcap3 = self.pkt2_cfg["capture"][0], self.pkt2_cfg["capture"][1], \
																		  self.pkt2_cfg["capture"][2], self.pkt2_cfg["capture"][3], \
																		  self.pkt2_cfg['capture'][4], self.pkt2_cfg['capture'][5]
		c_iface, c_num, c_pcap = self.pkt2_cfg["send"][0], self.pkt2_cfg["send"][1], self.pkt2_cfg["send"][2]
		read_name1, read_name2, read_name3, read_id = self.pkt2_cfg["read"][0], self.pkt2_cfg["read"][1], \
													  self.pkt2_cfg['read'][2], self.pkt2_cfg['read'][3]
		...
		for key in self.case2_step3:
			fun.cmd(self.case2_step3[key][0], 'gw')
			re = fun.cmd(self.case2_step3[key][1], 'gw')
			assert self.case2_step3[key][2] in re
