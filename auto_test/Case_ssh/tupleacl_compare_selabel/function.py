...
try:
	from tupleacl_compare_selabel import index
except Exception as err:
	print(
...
class Test_acl_compare_selabel():
	...
	def setup_class(self):
		...
		self.cap_pcap3 = self.pkt2_cfg["capture"][3]
	...
	@allure.feature('验证默认不放行时，匹配ACL策略的情况')
	...
	@allure.feature('验证配置了ACL策略时，优先匹配ACL策略，而不是seLabel')
	def test_acl_compare_selabel_a2(self):
	...
	# @pytest.mark.skip(reseason="skip")
	@allure.feature('验证没有配置ACL时，标记默认走selabel配置')
	def test_acl_compare_selabel_a3(self):
