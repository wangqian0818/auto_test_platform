...
class Test_qos_qbucket():
	...
	@allure.feature('验证qos对多桶（设置对每个会话生效）的限速情况')
	def test_qos_qbucket_a1(self):
		...
		speed_list = fun.qos_speed('qos_qbucket_a1.txt',s_txt)
		print(speed_list)
		for i in speed_list:
			print(i)
	...
	@allure.feature('验证qos对单桶（所有流量限速，不区分会话）的限速情况')
	def test_qos_qbucket_a2(self):
		...
		# 检查速率是否正确
		s_txt = fun.cmd('cat s.txt', 's')
		s_speed = fun.qos_speed('qos_qbucket_a2.txt', s_txt, qbucket='s')
		print(s_speed)
