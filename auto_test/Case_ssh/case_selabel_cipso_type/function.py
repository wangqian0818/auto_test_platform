...
try:
	from case_selabel_cipso_type import index
except Exception as err:
	print(
...
class Test_selabel_cipso_type():
	...
	@allure.feature('验证标记type字段匹配,配置边界设备type为1')
	...
	#@pytest.mark.skip(reseason="skip")
	@allure.feature('验证添加策略type字段左右边界')
	...
	# @pytest.mark.skip(reseason="skip")
	# @allure.feature('验证添加策略type正常字段1、2、5')
	# def test_selabel_cipso_type(self):
	#
	# 	# 下发配置并检查结果
	# 	for key in self.case2_step:
	# 		fun.cmd(self.case2_step[key][0], 'gw')
